...
async function getActiveSubscribers(): Promise<string[]> {
  try {
    ...
    return (parsed.subscribers || []).filter((sub: Subscriber) => sub.isActive).map((sub: Subscriber) => sub.email)
  } catch (error) {
...
async function getNewReleases(): Promise<SpotifyTrack[]> {
  try {
    const response = await fetch(`${process.env.VERCEL_URL || "http://localhost:3000"}/api/releases`)
...
async function getBuzzingSongs(): Promise<SpotifyTrack[]> {
  try {
    const response = await fetch(`${process.env.VERCEL_URL || "http://localhost:3000"}/api/buzzing`)
    if (!response.ok) throw new Error("Failed to fetch buzzing songs")
              ...
              <tr>
                <td style="background-color: #f8f9fa; padding: 30px; text-align: center; border-top: 1px solid #eee;">
                  ...
                  <table cellpadding="0" cellspacing="0" style="margin: 0 auto;">
                    <tr>
                      <td>
                        <a href="${process.env.VERCEL_URL || "https://afropulse.vercel.app"}" 
                           style="display: inline-block; background: linear-gradient(135deg, #ff6b35 0%, #f7931e 100%); 
                                  ...
                                  font-size: 14px; font-weight: 600; margin: 0 10px;">
                          Visit Afropulse
                        </a>
                      </td>
                    </tr>
...
export async function POST() {
  try {
    ...
    const [subscribers, newReleases, buzzingSongs] = await Promise.all([
    ...
    // Send emails with timeout and error handling
...
export async function GET() {
  try {
    ...
    const activeSubscribersCount = (parsed.subscribers || []).filter((sub: Subscriber) => sub.isActive).length

    return NextResponse.json({
      message: "Digest service is ready",
      activeSubscribers: activeSubscribersCount,
      totalSubscribers: parsed.total,
