...
export async function GET() {
  try {
    ...
    const [twitterBuzz, instagramBuzz, youtubeBuzz] = await Promise.all([
      scrapeTwitterBuzz(),
      scrapeInstagramBuzz(),
      scrapeYouTubeBuzz()
    ])
    ...
    const allBuzzData = [...twitterBuzz, ...instagramBuzz, ...youtubeBuzz]
    ...
    return NextResponse.json(
      { error: "Failed to fetch web buzz data" },
      { status: 500 }
    )
  }
}
